...
def sparkline_smooth(results, output=None, dmin=None, dmax=None, step=2, height=20, \
    ...
    im = Image.new("RGBA", ((len(results)-1)*step+4+20, height), 'white')
    im.putalpha(0)
    draw = ImageDraw.Draw(im)
    ...
    if has_last == True:
      ...
      draw.text([end[0]+3, height/2-5], str(results[-1]), fill="#888888")
    del draw 
    if output:
        im.save(output, "PNG")
        return output
...
if __name__ == "__main__":
    ...
    d = [ 171, 61, 73, 132, 147, 54, 295, 201, 133, 127, 224, 232, 271, 263, 78]
